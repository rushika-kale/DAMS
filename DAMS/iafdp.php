...
<!--<li onclick="TINY.box.show({iframe:'new2.html',boxid:'frameless',width:750,height:450,fixed:false,maskid:'bluemask',maskopacity:40,closejs:function(){closeJS()}})">iFrame, Blue Mask, Absolute Position, Frameless, Close Callback</li>-->
	...
	<body>
		<form name="data" action="inserttyfdp.php"  enctype="multipart/form-data" method="post">
			<label>Name<font color="red">*</font></label>
			<input type="text" class="size4" name="wname" placeholder="Enter FDP Name" data-validate="alpha,required"><br/>
...
function daydiff(first, second) {
    return (second-first)/(1000*60*60*24);
			...
			<br/><label>No. of days of FDP<font color="red">*</font></label>
			<input type="text" name="days" id="days" readonly data-validate="number,required">
			<br/><label><font>Joining Date</font></label>
			 <input type="text"name="jdate" id="join"data-validate="required"/></font></label>

			<br/><label>Place of FDP<font color="red" >*</font></label>
			<input type="text" class="size3" name="wvenue"  placeholder="Place name" data-validate="alphanumeric,required"><br/>
			<label>Registration fees:<font color="red"></font></label>
			<input type="text" class="size3" name="regfees"  placeholder="Rs." data-validate="number"><br/>
			<tr>
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			...
			/*$image = mysql_fetch_array(mysql_query("SELECT wattach1 FROM  faculty.fdp where username='$username' &&wname='$wname'"));
			...
			$image=$image['wattach1'];	
				echo"<font color='blue'>already uploaded</font>&nbsp;&nbsp;&nbsp;";
								...
								$image = mysql_fetch_array(mysql_query("SELECT wattach2 FROM  faculty.fdp where username='$username'"));
				...
				echo"<font color='blue'>already uploaded</font>&nbsp;&nbsp;&nbsp;";
							...
							$image = mysql_fetch_array(mysql_query("SELECT wattach3 FROM  faculty.fdp where username='$username'"));
