...
<head>
		...
		<div class="frame">
			<div class="data">
				<form name="data" onsubmit="return validForm() " method="post">
                	<div id="style1">
			<center>Organised seminar Programme				</center></div>
            <div class="right">
	...
	-->		<input type="button" onclick="window.open('<?php echo "examples/pdf7.php?workname=".$workname; ?>')" value="Download pdf">
			<!--<input type="button" onclick="TINY.box.show({iframe:'prworks.php',boxid:'frameless',width:850,height:550,fixed:false,maskid:'bluemask',maskopacity:40,closejs:function(){closeJS()}})"class="link1 id="menu_active""  value="Download excel ">-->
					...
					<ul class="tabs" data-persist="true">
						<li><a href="#view1">seminar Information</a></li>
...
?>
                                       ...
                                       <label><strong>Name of seminar<font color="red"></font></strong></label>
									<br/>
                                     <?php 
								$workname=$_GET['workname'];

									$name=mysql_fetch_array(mysql_query("SELECT name FROM faculty.organisedsem WHERE username='$username'&& name='$workname'"));
						$tech=mysql_fetch_array(mysql_query("SELECT tech_body FROM faculty.organisedsem WHERE username='$username'&& name='$workname'"));
						$spon=mysql_fetch_array(mysql_query("SELECT sponsor FROM faculty.organisedsem WHERE username='$username'&& name='$workname'"));
				        $role=mysql_fetch_array(mysql_query("SELECT role FROM faculty.organisedsem WHERE username='$username'&& name='$workname'"));	
                        $part=mysql_fetch_array(mysql_query("SELECT part_type FROM faculty.organisedsem WHERE username='$username'&& name='$workname'"));	
                          $count=mysql_fetch_array(mysql_query("SELECT count FROM faculty.organisedsem WHERE username='$username'&& name='$workname'"));		
									...
									echo' <label>'.$spon['sponsor'].'</label>';?> 
                                        <br/>
                                             ...
                                             <!--date-->
                                              <br/>
									<label><strong>Role in seminar</strong></label>
                                        <br/>         <?php
									echo' <label>'.$role['role'].'</label>';?> 
                                           
                                        <br/><br/>
                                        <label><strong>seminar for</strong></label>
                                        <br />
                                              <?php
							...
							<div id="view2">
							    ...
							    <td>                <?php
														$convenor=mysql_fetch_array(mysql_query("SELECT convenor FROM faculty.organisedsem WHERE username='$username'"));
							...
							<tr> <td><label><strong>Name of Co-convenor</strong></label></td>
							    <td>
                                                    <?php
														$coconvenor=mysql_fetch_array(mysql_query("SELECT coconvenor FROM faculty.organisedsem WHERE username='$username'"));
									echo':&nbsp; <label>'.$coconvenor['coconvenor'].'</label>';?></td>
                                    </tr>
                                    <tr><td><label><strong>Name of Coordinator</strong></label></td>
							    <td>
                                                    <?php
														$coor=mysql_fetch_array(mysql_query("SELECT coordinator FROM faculty.organisedsem WHERE username='$username'"));
									echo' :&nbsp; <label>'.$convenor['convenor'].'</label>';?></td></tr>
                                <tr>    <td>	<label><strong>Name of Co-coordinator</strong></label></td>
							    <td>
                                                    <?php
														$cocor=mysql_fetch_array(mysql_query("SELECT cocoordinator FROM faculty.organisedsem WHERE username='$username'"));
									echo' :&nbsp; <label>'.$cocor['cocoordinator'].'</label>';?></td></tr>
                                    <tr><td>
							 <label><strong>Name of Organisers</strong></label></td>
							    <td>
                                                    <?php
														$org=mysql_fetch_array(mysql_query("SELECT organiser FROM faculty.organisedsem WHERE username='$username'"));
									echo' :&nbsp; <label>'.$org['organiser'].'</label>';?></td></tr>
                                      <tr><td>
							 <label><strong>Organising Department</strong></label></td>
							    <td>
                                                    <?php
														$orgd=mysql_fetch_array(mysql_query("SELECT department FROM faculty.organisedsem WHERE username='$username'"));
                            ...
                            <div id="view3">
                            <?php
                            	$s1name=mysql_fetch_array(mysql_query("SELECT s1_name FROM faculty.organisedsem WHERE username='$username'&& name='$workname'"));
                                ...
                                <td><label><strong>Name Of Institute</strong></label></td>
                                  <td><?php
									$s1institute=mysql_fetch_array(mysql_query("SELECT s1_institute FROM faculty.organisedsem WHERE username='$username'&& name='$workname'"));					
									echo' :&nbsp; <label>'.$s1institute['s1_institute'].'</label>';?></td></tr>
                                
                                <tr>
                                <td><label><strong>Contact No.</strong></label></td>
                                  <td><?php
									$s1con=mysql_fetch_array(mysql_query("SELECT s1_contact FROM faculty.organisedsem WHERE username='$username'&& name='$workname'"));					
									echo' :&nbsp; <label>'.$s1con['s1_contact'].'</label>';?></td></tr>
                                
                                <tr>
                                <td><label><strong>Email ID</strong></label></td>
                                  <td><?php
									$s1email=mysql_fetch_array(mysql_query("SELECT s1_email FROM faculty.organisedsem WHERE username='$username'&& name='$workname'"));					
                                ...
                                <br /><br />
                                <?php
                            	$s2name=mysql_fetch_array(mysql_query("SELECT s2_name FROM faculty.organisedsem WHERE username='$username'&& name='$workname'"));
                                ...
                                <td><label><strong>Name Of Institute</strong></label></td>
                                  <td><?php
									$s2institute=mysql_fetch_array(mysql_query("SELECT s2_institute FROM faculty.organisedsem WHERE username='$username'&& name='$workname'"));					
									echo' :&nbsp; <label>'.$s2institute['s2_institute'].'</label>';?></td></tr>
                                
                                <tr>
                                <td><label><strong>Contact No.</strong></label></td>
                                  <td><?php
									$s2con=mysql_fetch_array(mysql_query("SELECT s2_contact FROM faculty.organisedsem WHERE username='$username'&& name='$workname'"));					
									echo' :&nbsp; <label>'.$s2con['s2_contact'].'</label>';?></td></tr>
                                
                                <tr>
                                <td><label><strong>Email ID</strong></label></td>
                                  <td><?php
									$s2email=mysql_fetch_array(mysql_query("SELECT s2_email FROM faculty.organisedsem WHERE username='$username'&& name='$workname'"));					
                                ...
                                <br /><br />
                                <?php
                            	$s3name=mysql_fetch_array(mysql_query("SELECT s3_name FROM faculty.organisedsem WHERE username='$username'&& name='$workname'"));
                                ...
                                <td><label><strong>Name Of Institute</strong></label></td>
                                  <td><?php
									$s3institute=mysql_fetch_array(mysql_query("SELECT s3_institute FROM faculty.organisedsem WHERE username='$username'&& name='$workname'"));					
									echo' :&nbsp; <label>'.$s3institute['s3_institute'].'</label>';?></td></tr>
                                
                                <tr>
                                <td><label><strong>Contact No.</strong></label></td>
                                  <td><?php
									$s3con=mysql_fetch_array(mysql_query("SELECT s3_contact FROM faculty.organisedsem WHERE username='$username'&& name='$workname'"));					
									echo' :&nbsp; <label>'.$s3con['s3_contact'].'</label>';?></td></tr>
                                
                                <tr>
                                <td><label><strong>Email ID</strong></label></td>
                                  <td><?php
									$s3email=mysql_fetch_array(mysql_query("SELECT s3_email FROM faculty.organisedsem WHERE username='$username'&& name='$workname'"));					
                                ...
                                ?>        
                                 <br /><br />
                                <?php
                            	$s4name=mysql_fetch_array(mysql_query("SELECT s4_name FROM faculty.organisedsem WHERE username='$username'&& name='$workname'"));
                                ...
                                <td><label><strong>Name Of Institute</strong></label></td>
                                  <td><?php
									$s4institute=mysql_fetch_array(mysql_query("SELECT s4_institute FROM faculty.organisedsem WHERE username='$username'&& name='$workname'"));					
									echo' :&nbsp; <label>'.$s4institute['s4_institute'].'</label>';?></td></tr>
                                
                                <tr>
                                <td><label><strong>Contact No.</strong></label></td>
                                  <td><?php
									$s4con=mysql_fetch_array(mysql_query("SELECT s4_contact FROM faculty.organisedsem WHERE username='$username'&& name='$workname'"));					
									echo' :&nbsp; <label>'.$s4con['s4_contact'].'</label>';?></td></tr>
                                
                                <tr>
                                <td><label><strong>Email ID</strong></label></td>
                                  <td><?php
									$s4email=mysql_fetch_array(mysql_query("SELECT s4_email FROM faculty.organisedsem WHERE username='$username'&& name='$workname'"));					
