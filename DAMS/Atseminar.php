...
<html>
	...
	<body>
		...
		<div class="frame">
				<div class="data">
			<div id="style1">
			<center>Attended Seminar Programmes				</center></div>
			<div class="left">
		
			<input type="button" onclick="TINY.box.show({iframe:'iasem.php',boxid:'frameless',width:850,height:550,fixed:false,maskid:'bluemask',maskopacity:40,closejs:function(){closeJS()}})"class="link1 id="menu_active"" value="Insert New Data">
			</div>			
			<div class="right">
	...
	-->		<input type="button" onclick="window.open('examples/pdf61.php')" value="Download pdf">
...
while($row = mysql_fetch_array($result))
  ...
  echo "<tr>";
 echo "<td>" . $sr ; ?> &nbsp;<a href='editor.php?user=<?php echo $username; ?>&title=<?php echo $row['wname'] ?>&type=<?php echo $type ?>'><img src='img/edit.png' height="20px" width="20px"></a><a href='deletevent.php?user=<?php echo $username; ?>&title=<?php echo $row['wname']; ?>&type=<?php echo "seminar"; ?>'><img src='img/delete.png' height="20px"width="20px"class='btnDelete'/></a></td><?php
  echo "<td>" . $row['wname'] . "</td>";
$dat = date("d-m-Y", strtotime($row['w_date']));
  echo "<td>" . $dat . "</td>";  echo "<td>".$row['days']."</td>";
