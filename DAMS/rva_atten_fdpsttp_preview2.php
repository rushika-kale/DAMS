  ...
  <head>
	...
	<?php include ('rva_navigation.php'); ?>
			...
			<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
			  ...
			  <h4 class="page-header"><center>FDP/STTP Report</center></h4>
						...
						<?php
								...
								echo '<th>Sr. No.</th>';
								echo '<th>Name</th>';
								if($_GET['fdpsttp']){echo '<th>Type of Event</th>';}
								...
								if(($range==1)||($range==2))
									$res=mysqli_query($con,"Select * from fdp where w_date BETWEEN '$from' AND '$to' 
														union
													  Select * from sttp where w_date BETWEEN '$from' AND '$to' 
													  order by username asc");
								else
									$res=mysqli_query($con,"Select * from fdp 
														union
													  Select * from sttp 
													  order by username asc");
								$i=1;
								...
								while($row=mysqli_fetch_array($res))
								{
									...
									if($fc==0)
									{
										...
										if(($range==1)||($range==2))
										{
											$ress=mysqli_query($con,"Select * from fdp where w_date BETWEEN '$from' AND '$to'
																union
															   Select * from sttp where w_date BETWEEN '$from' AND '$to'");
										}
										...
										{
										$ress=mysqli_query($con,"Select * from fdp
																union
															   Select * from sttp");
										}
										$i=1;
