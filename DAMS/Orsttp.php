...
<html>
	...
	<body>
		...
		<div class="frame">
				<div class="data">
			<div id="style1">
			<center>Organised STTP Programmes				</center></div>
			<div class="left">
		
			<input type="button" onclick="location.href='insertsttp.php'" value="Insert Data">
			</div>			
			<div class="right">
	...
	-->		<input type="button" onclick="window.open('examples/pdf12.php')" value="Download pdf">
...
$db=mysql_select_db("faculty",$con) or die("Failed to connect to MySQL: " . mysql_error());	
$username=$_SESSION['username1'];	
			$result = mysql_query("SELECT * FROM organisedsttp WHERE username='$username'");
...
while($row = mysql_fetch_array($result))
  ...
  echo "<tr>";
  echo "<td>" . $sr ; ?> &nbsp;<a href='editor.php?user=<?php echo $username; ?>&title=<?php echo $row['wname'] ?>&type=<?php echo $type ?>'><img src='img/edit.png' height="20px" width="20px"></a><a href='deletevent.php?user=<?php echo $username; ?>&title=<?php echo $row['name']; ?>&type=<?php echo "organisedsttp"; ?>'><img src='img/delete.png' height="20px"width="20px"class='btnDelete'/></a></td><?php
  echo "<td><a href='detailsttp.php?workname=".$row['name']."'>" . $row['name'] . "</a></td>";
