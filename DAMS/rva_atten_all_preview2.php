			...
			<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
			  ...
			  <h4 class="page-header"><center>Combined Report</center></h4>
						...
						<?php
								...
								echo '<th>Sr. No.</th>';
								echo '<th>Name</th>';
								if($_GET['event']){echo '<th>Type of Event</th>';}
								...
								if(($range==1)||($range==2))
									$res=mysqli_query($con,"
													Select userid,username,wname,wvenue,w_date,w_date2 from workshop where w_date BETWEEN '$from' AND '$to' 
													union
													Select userid,username,wname,wvenue,w_date,w_date2 from seminar where w_date BETWEEN '$from' AND '$to' 
													union
													Select userid,username,wname,wvenue,w_date,w_date2 from fdp where w_date BETWEEN '$from' AND '$to' 
													union
													Select userid,username,wname,wvenue,w_date,w_date2 from sttp where w_date BETWEEN '$from' AND '$to' 
													order by username asc");
								else
									$res=mysqli_query($con,"
													Select userid,username,wname,wvenue,w_date,w_date2 from workshop
													union
													Select userid,username,wname,wvenue,w_date,w_date2 from seminar
													union
													Select userid,username,wname,wvenue,w_date,w_date2 from fdp
													union
													Select userid,username,wname,wvenue,w_date,w_date2 from sttp 
													order by username asc");
								$i=1;
								$count=0;
								while($row=mysqli_fetch_array($res))
								{
									...
									if($_GET['event'])
									{
										...
										$choiceworkshop=mysqli_query($con,"Select * from workshop where w_date='$w_date' and w_date2='$w_date2' and wname='$wname'");
										$choiceseminar=mysqli_query($con,"Select * from seminar where w_date='$w_date' and w_date2='$w_date2' and wname='$wname'");
										...
										if(mysqli_num_rows($choiceworkshop))
											echo "<td style='vertical-align:middle;'>Workshop</td>";
										else if(mysqli_num_rows($choiceseminar))
